...
const signupUser = async (id, password, confirmPassword, email, department, grade, name, studentIdImageUrl) => {
  validatePassword(password, confirmPassword);  // 비밀번호 유효성 검사
...
const loginUser = async (id, password) => {
  ...
  if (!user) {
    throw createError('USER_NOT_FOUND', 404);  // 사용자가 없음
  }

  if (user.admin === 'pending') {
    throw createError('USER_PENDING', 402);  // 승인 대기 중인 사용자
  }

  const passwordMatch = await bcrypt.compare(password, user.password);  // 비밀번호 일치 확인

  if (!passwordMatch) {
    throw createError('INVALID_PASSWORD', 401);  // 비밀번호 불일치
  }

  if (user.admin === 'rejected') {
    const rejectionReason = user.rejection_reason || '관리자에게 문의하세요.';
    const error = createError(`USER_REJECTED:${rejectionReason}`, 403);
 ...
 */
const editUserInfo = async (userId, editedUserInfo) => {
  const user = await userModel.getUserById(userId);  // 사용자 조회
 ...
 * @returns {Promise<object>} 사용자 정보 객체.
 */
const getUserInfoWithSalesAndBalance = async (userId) => {
  // 사용자 정보 조회
...
module.exports = {
  ...
  changePassword,
  editUserInfo,
  getUserInfoWithSalesAndBalance,
};
